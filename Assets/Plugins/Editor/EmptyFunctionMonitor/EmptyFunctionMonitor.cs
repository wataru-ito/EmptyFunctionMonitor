...
namespace EmptyFunctionMonitor
{
	...
	public class EmptyFunctionMonitor : EditorWindow
	{
		...
		void OnEnable()
		{
			titleContent = new GUIContent("空関数消す造");
		...
		void OnGUI()
		{
			...
			if (_result == null)
			{
				EditorGUILayout.HelpBox("検索を実行してください", MessageType.Info);
				return;
			}
			...
			EditorGUILayout.HelpBox("ダブルクリックで開く", MessageType.Info);
		}
		...
		readonly GUIContent kSearchContent = new GUIContent("検索", "全てのスクリプトから空関数を検索");
